...
$id = $brand = $model = $car_age = $availability = $car_image_path = $price_per_day = $transmission = $fuel_type = '';

if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    $car_id = $_GET['id'];

    $sql = "SELECT id, brand, model, car_age, availability, car_image_path, price_per_day, transmission, fuel_type FROM cars WHERE id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $car_id);
    $stmt->execute();
    $stmt->store_result();

    $stmt->bind_result($id, $brand, $model, $car_age, $availability, $car_image_path, $price_per_day, $transmission, $fuel_type);
...
<body>
    ...
    <section class="page-section bg-light text-dark mb-0 mt-2" id="editcar">
        <div class="container">
            <div class="row">
                            ...
                            <form action="update_car.php" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" class="form-control" id="car_age" name="car_age"
                                        value="<?php echo $car_age; ?>" required>
                                </div>
                                <div class="form-group">
                                    <label for="availability">Availability</label>
                                    <select class="form-control" id="availability" name="availability" required>
                                        <option value="1" <?php echo ($availability == 1) ? 'selected' : ''; ?>>Available
                                        </option>
                                        <option value="0" <?php echo ($availability == 0) ? 'selected' : ''; ?>>Not
                                            Available</option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" class="form-control" id="price_per_day"
                                        name="price_per_day" value="<?php echo $price_per_day; ?>" required>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    <label for="car_image">Upload New Car Image</label>
                                ...
                                <a href="carlist.php" class="btn btn-secondary">Cancel</a>
                            </form>
                        </div>
                    </div>
                </div>
